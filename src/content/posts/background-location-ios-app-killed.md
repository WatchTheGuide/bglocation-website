...
The user explicitly terminates the app from the app switcher. Most developers assume this is the end of the road — if the user killed the app, GPS is dead.

In practice, that's the realistic expectation. iOS does expose a Significant Location Change API that *can* relaunch a terminated app silently in the background — but using it correctly requires a native layer that persists its full tracking state (HTTP endpoint, configuration, was-tracking flag) across launches, re-initializes on cold start, and resumes the GPS session within the ~10-second background window iOS grants. Most plugins don't do this — and a JavaScript-based tracker fundamentally can't, because the JS runtime takes too long to boot.

The practical takeaway: design your product around the assumption that swipe-to-kill ends the session. If you need true terminated-app resumption, validate that your chosen plugin actually persists and restores state — don't assume the SLC API is enough on its own.
...
For background tracking to work reliably — surviving minimize, surviving OS-driven suspends, getting any chance of relaunches — your app needs the "Always" location authorization.

"While Using" authorization does allow some background location — the app continues receiving GPS updates after it's minimized, as long as it was actively tracking when it went to the background. iOS shows a blue pill indicator at the top of the screen to signal this. But "While Using" has a hard limit: if the app is killed — either by the user or by the OS — iOS will not wake it up on movement. The tracking session is gone.

"Always" authorization is what makes Significant Location Change wake-ups possible at the OS level. Whether your native layer turns that wake-up into a resumed tracking session is a separate problem (see failure mode 2). But without "Always," even a perfect native implementation has nothing to work with.
...
A note on terminated-app resumption: bglocation registers Significant Location Change monitoring so iOS *can* wake the app on movement after termination. Fully automatic resume of the tracking session after a user-initiated kill — where the native layer rehydrates its HTTP config and tracking state on the cold launch — is on the roadmap, not in the current release. For now, treat the swipe-to-kill scenario as ending the session, the same way you would with any other current plugin.
...
2. Lock the screen and put the device down
3. Move around for several minutes (a walk, a drive)
4. Reopen the app — your event log and your backend should both show continuous updates from the period the screen was locked
...
Testing the terminated-app scenario (kill the app, move, verify silent resume) is a separate exercise that requires the native layer to fully persist and restore state on cold launch. As noted earlier, that path is plugin-specific — verify what your chosen plugin actually does before relying on it.
...
- [ ] **Tested on device**: Not just simulator — physical iPhone, with the screen locked and the app running in the background for an extended period
- [ ] **Decided on the terminated scenario**: Either accept that swipe-to-kill ends the session, or validate that your plugin actually rehydrates state on a cold SLC-triggered launch
